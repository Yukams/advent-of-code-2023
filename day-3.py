...
x = """.....664...998........343...............851............................2............414.....................3....................948.164....
...
# PART ONE : res = [[] for i in range(nbLine)]
for line in range(nbLine):
	for col in range(nbCol):
		# If we find a symbol
		# PART ONE : if(xSplit[line][col] != "." and not xSplit[line][col].isdigit()):
		if(xSplit[line][col] == "*"):
			startLine = line if line == 0 else line-1
			startCol = col if col == 0 else col-1
			endLine = line if line == len(xSplit)-1 else line+1
			endCol = col if line == len(xSplit)-1 else col+1

			"""
			PART ONE

			# Retrieve any UNIQUE index of a number around the symbol
			for i in range(startLine, endLine+1):
				for j in range(startCol, endCol+1):
					if(xSplit[i][j].isdigit() and j not in res[i]):
						res[i].append(j)
		 """
			
			# PART TWO
			# ====== Count the number of numbers around a symbol
			nbNumber = 0
			res = []
			for i in range(startLine, endLine+1):
				lineRes = []
				isNewNumber = True
				for j in range(startCol, endCol+1):
					if(xSplit[i][j].isdigit() and isNewNumber):
						lineRes.append(j)
						nbNumber += 1
						isNewNumber = False
					elif(not xSplit[i][j].isdigit() and not isNewNumber):
						isNewNumber = True
				res.append(lineRes)

			# Retrieve full numbers around the symbol if there are exacty two numbers around it
			if nbNumber == 2:
				product = 0
				for i in range(len(res)):
					...
					for nbIdx in res[i]:
						findLowerBound = nbIdx
						findUpperBound = nbIdx
						while(findLowerBound > 0 and xSplit[lineIdx][findLowerBound-1].isdigit()):
							findLowerBound -= 1
						while(findUpperBound < nbCol-1 and xSplit[lineIdx][findUpperBound+1].isdigit()):
							findUpperBound += 1
	
						# Construct the string number
						stringNumber = ""
						for pointer in range(findLowerBound, findUpperBound+1):
							stringNumber += xSplit[lineIdx][pointer]

						# Retrieve gear ratio
						product = int(stringNumber) if product==0 else product*int(stringNumber)
				sumRes += product
...
# Remove idx from the same number
for i in range(len(res)):
	newResLine = []
	if len(res[i]) > 0:
		for j in range(len(res[i])-1):
			if(res[i][j]+1 != res[i][j+1]):
				newResLine.append(res[i][j])
		newResLine.append(res[i][-1])
	res[i] = newResLine
#print(res)

# Get each full number from one of their digit index
numberListRes = []
sumRes = 0
for idxListIndex in range(nbLine):
	lineNumberList = []
	idxList = res[idxListIndex]

	# Retrieve numbers from the original list
	for idxDigit in idxList:
		findLowerBound = idxDigit
		findUpperBound = idxDigit
		while(findLowerBound > 0 and xSplit[idxListIndex][findLowerBound-1].isdigit()):
			findLowerBound -= 1
		while(findUpperBound < nbCol-1 and xSplit[idxListIndex][findUpperBound+1].isdigit()):
			findUpperBound += 1

		# Construct the string number
		stringNumber = ""
		for pointer in range(findLowerBound, findUpperBound+1):
			stringNumber += xSplit[idxListIndex][pointer]

		# Finally get the number in the list (for debug)
		#lineNumberList.append(int(stringNumber))
		# Get the number in the list
		sumRes += int(stringNumber)
		
	numberListRes.append(lineNumberList)
