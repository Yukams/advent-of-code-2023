x = ["14gxqgqsqqbxfpxnbccjc33eight",
"eight2sevenkl",
...
for code in x:
	fIdx = 0
	fNumber = None
	while(fNumber is None):
		if(code[fIdx].isdigit()):
			fNumber = int(code[fIdx])
		else :
			fNumber = getNumberFromText(code[:fIdx+1])
		fIdx += 1

		lIdx = len(code) - 1
		lNumber = None
		while(lNumber is None):
			if(code[lIdx].isdigit()):
				lNumber = int(code[lIdx])
			else:
				lNumber = getNumberFromText(code[lIdx:])
			lIdx -= 1
	
	res+=int(str(fNumber)+str(lNumber))
