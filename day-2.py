...
y = """Game 1: 10 green, 9 blue, 1 red; 1 red, 7 green; 11 green, 6 blue; 8 blue, 12 green
...
xNbCubes = {'red':12, 'green':13, 'blue':14}
yNbCubes = {'red':12, 'green':13, 'blue':14}

res = []
for line in y.split('\n'):
	cubes = {}
	for round in line.split(':')[1].split(";"):
		for cubeDetails in round.split(','):
			_, number, color = cubeDetails.split(' ')
			if(color in cubes):
...
res = [i for i, x in enumerate(res) if
	set(x.keys()).issubset(yNbCubes.keys()) and
	x['red'] <= yNbCubes['red'] and
	x['blue'] <= yNbCubes['blue'] and
	x['green'] <= yNbCubes['green']]
...
sumRes = 0
for game in res:
	sumRes += (game['blue'] * game['red'] * game['green'])
